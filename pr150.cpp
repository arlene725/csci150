...
int getit(double x);
void writesched(vector<course*> v, ofstream f);
...
void writesched(vector<course*> v, ofstream f) {
	...
	//METHOD THAT SORTS GOES HERE
	for (int i = 0; i < y.size(); ++i) {
		f << "\t" << y.at(i)->getid() << "\t" << y.at(i)->getbegtime() << "-" << y.at(i)->getendtime() << endl;
	}
	y.clear();
	f << "Tuesday:" << endl;
	for (int i = 0; i < v.size(); ++i) {
